...
./alpine -r . -b /dev -b /sys -b /proc -b /tmp \
  ...
  wget https://cdn.bosd.io.vn/windows11.qcow2 && mv windows11.qcow2 /home/container \
...
run_system() {
  ...
  $DOCKER_RUN "qemu-system-x86_64 -m "16384" -smp $(nproc --all) -cpu EPYC-Milan -nic user,hostfwd=tcp::"25414"-:3389 -drive file=windows11.qcow2 -drive file=fat:rw:shared -usbdevice tablet -display vnc=127.0.0.1:1,password=on -monitor stdio < qemu_cmd.txt"                         
